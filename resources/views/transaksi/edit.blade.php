...
<div class="container">
    ...
    <form action="{{ route('transaksi.update', $transaksi) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select name="pelanggan_id" id="pelanggan_id" class="form-control" required>
                ...
                @foreach($pelanggans as $pelanggan)
                    <option value="{{ $pelanggan->id }}" {{ old('pelanggan_id', $transaksi->pelanggan_id) == $pelanggan->id ? 'selected' : '' }}>{{ $pelanggan->nama }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <select name="user_id" id="user_id" class="form-control" required>
                ...
                @foreach($users as $user)
                    <option value="{{ $user->id }}" {{ old('user_id', $transaksi->user_id) == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <select name="barang_id" id="barang_id" class="form-control" required>
                ...
                @foreach($barangs as $barang)
                    <option value="{{ $barang->id }}" {{ old('barang_id', $transaksi->barang_id) == $barang->id ? 'selected' : '' }}>{{ $barang->nama }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <input type="datetime-local" name="waktu_sewa_mulai" id="waktu_sewa_mulai" class="form-control" required value="{{ old('waktu_sewa_mulai', date('Y-m-d\TH:i', strtotime($transaksi->waktu_sewa_mulai))) }}">
        ...
        <div class="mb-3">
            ...
            <input type="datetime-local" name="waktu_sewa_selesai" id="waktu_sewa_selesai" class="form-control" required value="{{ old('waktu_sewa_selesai', date('Y-m-d\TH:i', strtotime($transaksi->waktu_sewa_selesai))) }}">
